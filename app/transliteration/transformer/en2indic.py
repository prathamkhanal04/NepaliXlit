...
MODEL_DOWNLOAD_URL = "https://github.com/Supriya090/NepaliXlit/releases/download/v1.0/nepalixlit-en-ne.zip"
DICTS_DOWNLOAD_URL = (
...
class XlitEngineTransformer_En2Indic(BaseEngineTransformer):
    ...
    def translit_word(
        self, word, lang_code="default", topk=4, transliterate_numerals=False
    ):

        if lang_code in self.tgt_langs:
            transliterated_word_list = self._transliterate_word(
                word,
                src_lang="en",
                tgt_lang=lang_code,
                topk=topk,
                nativize_numerals=transliterate_numerals,
            )
    ...
    def translit_sentence(
        self, eng_sentence, lang_code="default", transliterate_numerals=True
    ):

        if lang_code in self.tgt_langs:
            return self._transliterate_sentence(
                eng_sentence,
                src_lang="en",
                tgt_lang=lang_code,
                nativize_numerals=transliterate_numerals,
            ...
            res_dict = {}
            for la in self.tgt_langs:
                res_dict[la] = self._transliterate_sentence(
                    eng_sentence,
                    src_lang="en",
                    tgt_lang=la,
                    nativize_numerals=transliterate_numerals,
